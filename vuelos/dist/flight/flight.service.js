...
let VuelosService = class VuelosService {
    constructor(vuelosModelos) {
        this.vuelosModelos = vuelosModelos;
    }
    async insertar(flightDTO) {
    ...
    async actualizar(id, flightDTO) {
        return await this.vuelosModelos.findByIdAndUpdate(id, flightDTO, { new: true });
    }
    async eliminar(id) {
        await this.vuelosModelos.findByIdAndDelete(id);
        return { status: common_1.HttpStatus.OK, msg: 'Eliminado' };
    }
    async insertarPasajero(flightDTO, pasajeroId) {
